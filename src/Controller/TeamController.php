...
class TeamController extends AbstractController
{
    ...
    public function index(): Response
    {
        ...
        $limit = 5;
        $teams = $this->getDoctrine()->getRepository(Team::class)->getTeamsByPagination($thisPage,$limit);
        $maxPages = ceil($teams->count() / $limit);
    ...
    public function add_team(): Response
    {
    ...
    public function store_team(Request $request): Response
    {
        $submittedToken = $request->request->get('token');
        if ($this->isCsrfTokenValid('store-item', $submittedToken)) {
            $params = $request->request->all();
            $date = new \DateTime('@'.strtotime('now'));
            $team = new Team();
            $team->setName($params['teamName']);
            $team->setCountry($params['countryName']);
            $team->setBalance($params['teamBalance']);
            $team->setCreatedAt($date);
            $this->getDoctrine()->getRepository(Team::class)->add($team,true);
    ...
    public function team_players($id): Response
    {
        if(isset($_GET['page'])){
            $thisPage = $_GET['page'];
        }else{
        ...
        $limit = 3;
        $Players = $this->getDoctrine()->getRepository(Player::class)->getPlayersByPagination($thisPage,$limit,$id);

        $maxPages = ceil($Players->count() / $limit);
    ...
    public function add_player($id): Response
    {
        return $this->render('team/add_players.html.twig', [
            'controller_name' => 'TeamController',
            'teamId' => $id
        ]);
    }

    public function store_player(Request $request): Response
    {
        $submittedToken = $request->request->get('token');
        if ($this->isCsrfTokenValid('store-item', $submittedToken)) {
            $params = $request->request->all();
            ...
            $player->setSurname($params['surName']);
            $player->setTeamId($params['teamId']);
            $player->setCreateAt($date);
            $this->getDoctrine()->getRepository(Player::class)->add($player,true);
            $this->get('session')->getFlashBag()->add('success', 'Player Added Successfully');
            return $this->redirectToRoute('add_player',['id'=>$params['teamId']]);
        }
    }
